...
#to check whther given folder information   
    Ret = False
    Ret = os.path.exists(DirectoryPath)#to check whether the name is existed or not
    if (Ret == False):
        print("Marvellous automation error : No such directory with name ", DirectoryPath)
        return 
    
    Ret = os.path.isdir(DirectoryPath)#to check whether the directory is there or not
    
    if (Ret == False):
        print("Marvellous Automation Error : Its not a directory with name ",DirectoryPath)
        return
